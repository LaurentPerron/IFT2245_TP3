...
{
  if (page_table[page_number].frame_number > 0 && page_table[page_number].valid == true) return page_table[page_number].frame_number;
  return -1;
}
...
/* Renvoie true si `page_number` est `readonly`.  */
bool pt_readonly_p (unsigned int page_number)
{
  if (page_table[page_number].readonly == true) return true;
  return false;
}
